...
class LeNetEnv:
    def __load_dataset(self):
        ...
        for i in range(60000):
            lst.append(cv2.resize(x_train[i, :, :], (32, 32)))
        ...
        self.predict_data_mean = tf.reduce_mean(self.predict_data) * 3.
    ...
    def __init__(self):
        ...
        tf_model = ModelJSON('./tf_lenet-5.json')
        torch_model = ModelJSON('./torch_lenet-5.json')
        ...
        with open('./tf_lenet-5.json', 'r') as f:
...
def save_meta_with_weights(tf_json, reward, err_type, c, err_info=None):
    ...
    if err_type == 'CRASH':
        # crash样本 保存错误信息
        with open('./pool/crash/{0}'.format(c), 'w+') as f:
            f.write(err_info)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        rand1 = random.randint(6, 24)
        conv2d_1['out_channels'] = rand1
        # layer 1
        avg_pool_1 = network[1]['params']
        avg_pool_1['pool_size'] = pool_sizes[random.randint(0, 6)]
        # layer 2
        conv2d_2 = network[2]['params']
        rand2 = random.randint(16, 64)
        conv2d_2['in_channels'] = rand1
        conv2d_2['out_channels'] = rand2
        # layer 3
        avg_pool_2 = network[3]['params']
        avg_pool_2['pool_size'] = pool_sizes[random.randint(0, 6)]
        # layer 4
        conv2d_3 = network[4]['params']
        rand3 = random.randint(120, 480)
        conv2d_3['in_channels'] = rand2
        conv2d_3['out_channels'] = rand3
        # layer 6
        dense_1 = network[6]['params']
        dense_1['in_features'] = rand3
        rand4 = random.randint(84, 336)
        dense_1['out_features'] = rand4
        # layer 7
        dense_2 = network[7]['params']
        dense_2['in_features'] = rand4
        ...
        try:
            ...
            tf_phased_train = tf_train(tf_model.network, optimizer, "CrossEntropy", ln.epochs,
                                       ln.batch_size, ln.tf_dataset, shuffle=False)
            torch_phased_train = torch_train(torch_model.network, optimizer, "CrossEntropy", ln.epochs,
                                             ln.batch_size, ln.torch_dataset, shuffle=False)
